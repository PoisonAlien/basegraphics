...
lo = layout(mat = matrix(data = c(1,2,3), ncol = 3, byrow = FALSE), widths = c(2, 5, 2))
...
```{r}
lo = layout(mat = matrix(data = c(1,2,3), ncol = 3, byrow = FALSE), widths = c(1, 5, 2))
...
for(i in seq_along(data$Performance)){
  ...
  text(x = 0.1, y = i, labels = data$Company[i], font= 2, pos= 4, cex = 1.1, col = "#2c3e50")
}
...
for(i in seq_along(data$Performance)){
  rect(xleft = 0, ybottom = i - 0.4, xright = data$Performance[i], ytop = i + 0.4, col = barcols[i], border = NA)  
  text(x = data$Performance[i], y = i, labels = data$Performance[i], pos = ifelse(test = data$Performance[i] < 0, 2, 4), col = barcols[i], font = 2)
}
...
text(x = 30, y = 6, label = desc, pos = 4, cex = 1.2, font = 2)
...
symbols(
  x = rep(0.5, nrow(data)),
  y = seq_along(data$Company),
  circles = rev(sqrt(abs(data$Market_cap / pi))),
  add = TRUE, inches = 0.6, bg = adjustcolor(col = barcols, 0.6), fg = "white", xpd = TRUE)

for(i in seq_along(data$Performance)){
  #rect(xleft = 0, ybottom = i - 0.4, xright = data$Performance[i], ytop = i + 0.4, col = barcols[i], border = NA)  
  text(x = 0, y = i, labels = paste0("$", data$Market_cap[i], "B"), font= 2, pos= 4, cex = 1.7, col = barcols[i])
}
text(y = 11.4, x = 0.5, labels = c("MARKET CAP\nUSD"), col =c("#34495e"), cex = 0.6, font = 2, xpd= TRUE)

abline(h = b, lty = 2, col= "gray")
```
...
text(x = 10, y = 7.5, label = desc, pos = 4, cex = 0.6, font = 2)
